...
int main(int argc, char *argv[])
{
	...
	ratetmp[0]='\0';
        ...
        /* Scan CLI parameters */
        while ((c = getopt(argc, argv, "hi:m:d:n:s:p:z:f:")) != -1) {
                switch(c) {
                case 'h': usage(); break;
...
void usage(void)
{
        ...
        printf("                        -s \"<startsize stopsize stepsize> -p <seconds between steps> (please note that checksum in not recalculated!!!)\" \n");
        printf("                        -z \"<startrate stoprate steprate) -p <seconds between steps> \" \n");
        ...
        printf("                                                                             	 - send 10s packets with this packet length, increase packet length by 100 bytes till 1500 \n");
        printf("  ./packETHcli -i eth1 -m 2  -z \"10000 100000 10000\" -p 10 -f packet3.pcap   - send packet2.pcap with 10000kbit/s for 10s, then increse rate by 10000kbit/s and \n");
        printf("                                                                             	 - send with this rate for 10s. Continue until 1000000kbit/s\n");
...
int two(char *iftext, long delay, long pkt2send, char* filename, char *sizetmp, int period, char *ratetmp) {
	...
	if (strlen(ratetmp) > 0 ) {
		...
		if (startrate > stoprate) {
			printf("\nstartrate is greater than stoprate\n\n");
			return 1;
		}
		if (startrate < 1) {
